...
# Player2 (AI using A*)
def player2_astar(difficulty):
    start = player2.centery
    goal = ball.centery

    # دالة التقدير (heuristic) – المسافة المطلقة
    def heuristic(pos):
        return abs(pos - goal)

    open_set = []
    heappush(open_set, (heuristic(start), 0, start, []))  # (f(n), g(n), current, path)
    visited = set()

    while open_set:
        f, g, current, path = heappop(open_set)

        if abs(current - goal) <= 5:  # Stop if the AI is close to the ball
            break

        for move in [-5, 5]:
            new_pos = current + move
            if play_area.top < new_pos < play_area.bottom and new_pos not in visited:
                visited.add(new_pos)
                new_path = path + [move]
                heappush(open_set, (g + 1 + heuristic(new_pos), g + 1, new_pos, new_path))

    if path:
        move_distance = path[0]

        # Adjust difficulty level
        if difficulty == 1:
            move_distance = move_distance // 2  # Slow movement for easy difficulty
        elif difficulty == 2:  # Medium difficulty
            move_distance = move_distance  # Keep normal speed
        elif difficulty == 3:
            move_distance = move_distance * 2  # Faster movement for hard difficulty
        
        player2.y += move_distance
...
# Main game loop
game_over = False
difficulty_level = 3  # Default difficulty level (1: Easy, 2: Medium, 3: Hard)
...
while True:
    ...
    if not game_over:
        ball_animation()
        player1_animation()
        player2_astar(difficulty_level)  # AI movement
